    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            This will mark the request as completed, create assets for all items, and generate an invoice.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                {isLoadingWarehouses ? (
                  <div className="p-2 text-xs text-muted-foreground">Loading warehouses...</div>
                ) : warehouses?.data ? (
                  ...
                  <div className="p-2 text-xs text-muted-foreground">No warehouses found</div>
                )}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger id="zone">
                <SelectValue placeholder={!warehouseId ? "Select warehouse first" : "Select zone..."} />
              </SelectTrigger>
              <SelectContent>
                {isLoadingZones ? (
                  <div className="p-2 text-xs text-muted-foreground">Loading zones...</div>
                  ...
                  <div className="p-2 text-xs text-muted-foreground">
                    {warehouseId ? "No zones found for this company in this warehouse" : "Select warehouse first"}
                  </div>
                )}
              </SelectContent>
            </Select>
            {warehouseId && zones?.data && zones.data.length === 0 && (
              <p className="text-xs text-destructive mt-1">
                No zones assigned to this company in the selected warehouse. Please assign a zone first.
              </p>
            )}
          </div>
        </div>
