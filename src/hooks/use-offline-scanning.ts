...
export function useOfflineOutboundScanProgress(orderId: string | null) {
  ...
  const cachedQuery = useQuery({
    ...
    queryFn: async () => {
      const { getCachedScanProgress, getPendingScans } = await import('@/lib/offline/offline-storage');
      ...
      const totalScanned = cached.data.itemsScanned + 
        pendingScans.reduce((sum, s) => sum + s.quantity, 0);
      ...
      return {
        data: {
          ...
          percent_complete: Math.min(100, Math.round((totalScanned / cached.data.totalItems) * 100)),
          assets: mergedAssets,
        },
  ...
  return network.isOnline ? onlineQuery : {
    ...cachedQuery,
...
export function useOfflineScanOutboundItem() {
  ...
  const offlineMutation = useMutation({
    mutationFn: async (data: { orderId: string; qrCode: string; quantity?: number }) => {
      const { savePendingScan, getCachedScanProgress } = await import('@/lib/offline/offline-storage');
      ...
      return {
        data: {
          asset: {
            ...
            required_quantity: asset.requiredQuantity,
            remaining_quantity: asset.requiredQuantity - asset.scannedQuantity - (data.quantity || 1),
          },
          progress: {
            total_items: cached.data.totalItems,
            items_scanned: cached.data.itemsScanned + (data.quantity || 1),
            percent_complete: Math.min(100, cached.data.percentComplete + 
      ...
      return {
        data: {
          asset: {
            ...
            remaining_quantity: (asset?.requiredQuantity || 0) - (asset?.scannedQuantity || 0) - (data.quantity || 1),
            condition: data.condition,
          ...
          progress: {
            ...
            percent_complete: Math.min(100, (cached?.data.percentComplete || 0) + 
              Math.round(((data.quantity || 1) / (cached?.data.totalItems || 1)) * 100)),
          },
        },
        success: true,
        offline: true,
