...
export const getOrderPrice = (orderPricing: Partial<OrderPricing> | null | undefined) => {
    ...
    const basePrice = Number(orderPricing?.base_ops_total ?? 0) + (Number(orderPricing?.base_ops_total ?? 0) * (marginAmount / 100));
    const transportPrice = Number(orderPricing?.transport?.final_rate ?? 0) + (Number(orderPricing?.transport?.final_rate ?? 0) * (marginAmount / 100));
    const catalogPrice = Number(orderPricing?.line_items?.catalog_total ?? 0) + (Number(orderPricing?.line_items?.catalog_total ?? 0) * (marginAmount / 100));
