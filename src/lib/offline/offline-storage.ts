...
async function calculateStorageStats(): Promise<OfflineMetadata> {
  ...
  const totalSizeBytes = photos.reduce((sum, p) => sum + p.sizeBytes, 0) +
...
export async function savePendingOrder(orderData: PendingOrder['orderData']): Promise<PendingOrder> {
  const db = await getDB();
...
export async function deleteSyncedOrders(): Promise<void> {
  ...
  await Promise.all([
    ...orders.map(o => tx.store.delete(o.id)),
    tx.done,
  ]);
}
...
export async function savePendingScan(scanData: Omit<PendingScanEvent, 'id' | 'localId' | 'status' | 'createdAt' | 'retryCount'>): Promise<PendingScanEvent> {
  const db = await getDB();
  const id = generateLocalId();
  ...
  // Add to sync queue
  const endpoint = scanData.scanType === 'OUTBOUND'
...
export async function clearExpiredCache(): Promise<void> {
  ...
  await Promise.all([
    ...toDelete.map(c => tx.store.delete(c.orderId)),
    tx.done,
  ]);
}
...
async function addToSyncQueue(item: Omit<SyncQueueItem, 'id' | 'status' | 'createdAt' | 'retryCount' | 'maxRetries'>): Promise<void> {
  const db = await getDB();
  const id = generateLocalId();
...
export async function updateSyncQueueItem(
  id: string,
  updates: Partial<Pick<SyncQueueItem, 'status' | 'lastAttemptAt' | 'retryCount' | 'errorMessage'>>
): Promise<void> {
  const db = await getDB();
...
export async function clearCompletedSyncItems(): Promise<void> {
  ...
  const toDelete = items.filter(i => 
    i.status === 'pending' && i.retryCount >= i.maxRetries
  );
  
  const tx = db.transaction('syncQueue', 'readwrite');
  await Promise.all([
    ...toDelete.map(i => tx.store.delete(i.id)),
    tx.done,
  ]);
}
