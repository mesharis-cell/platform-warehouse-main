...
const PICKUP_STATUS_CONFIG: Record<string, { label: string; color: string }> = {
    ...
    READY_FOR_PICKUP: { label: "Ready for Pickup", color: "bg-emerald-100 text-emerald-700 border-emerald-300" },
    ...
    AWAITING_RETURN: { label: "Awaiting Return", color: "bg-amber-100 text-amber-700 border-amber-300" },
...
export default function WarehouseSelfPickupDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!pickup) return <div className="text-center py-12 text-muted-foreground">Self-pickup not found</div>;

    const sc = PICKUP_STATUS_CONFIG[pickup.self_pickup_status] || { label: pickup.self_pickup_status, color: "bg-gray-100 text-gray-700" };
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Link href="/self-pickups"><Button variant="ghost" size="icon"><ArrowLeft className="h-4 w-4" /></Button></Link>
                    <div>
                        <h1 className="text-2xl font-bold">{pickup.self_pickup_id}</h1>
                        <p className="text-sm text-muted-foreground">{(pickup.company as any)?.name}</p>
                    </div>
                ...
                <div className="flex gap-2">
                    ...
                    {pickup.self_pickup_status === "READY_FOR_PICKUP" && (
                        <Link href={`/scanning/self-pickup-handover/${id}`}><Button>Start Handover Scan</Button></Link>
                    )}
                    {pickup.self_pickup_status === "AWAITING_RETURN" && (
                        <Link href={`/scanning/self-pickup-return/${id}`}><Button>Start Return Scan</Button></Link>
                    )}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    <Card>
                        <CardHeader><CardTitle className="text-base">Collector Details</CardTitle></CardHeader>
                        <CardContent className="space-y-3">
                            <div className="flex items-center gap-2"><User className="h-4 w-4 text-muted-foreground" /><span className="font-medium">{pickup.collector_name}</span></div>
                            <div className="flex items-center gap-2"><Phone className="h-4 w-4 text-muted-foreground" /><span>{pickup.collector_phone}</span></div>
                            {pickup.collector_email && <div className="flex items-center gap-2"><Mail className="h-4 w-4 text-muted-foreground" /><span>{pickup.collector_email}</span></div>}
                            {pw && <div className="flex items-center gap-2"><Clock className="h-4 w-4 text-muted-foreground" /><span>Pickup: {new Date(pw.start).toLocaleString()} - {new Date(pw.end).toLocaleString()}</span></div>}
                            {pickup.expected_return_at && <div className="flex items-center gap-2"><Clock className="h-4 w-4 text-muted-foreground" /><span>Expected return: {new Date(pickup.expected_return_at).toLocaleString()}</span></div>}
                        </CardContent>
                    </Card>
                                ...
                                {items.map((item: any) => (
                                    <div key={item.id} className="flex items-center justify-between p-3 border rounded-lg">
                                        <div>
                                            ...
                                            <p className="text-sm text-muted-foreground">Qty: {item.quantity} | Vol: {item.total_volume} m3</p>
                                        </div>
                                        <Badge variant="outline"><Package className="h-3 w-3 mr-1" />{item.quantity}</Badge>
                                    </div>
                                ))}
                            </div>
                        </CardContent>
                        ...
                        <CardContent>
                            <div className="space-y-4">
                                {history.map((entry: any) => {
                                    const ec = PICKUP_STATUS_CONFIG[entry.status] || { label: entry.status, color: "bg-gray-100 text-gray-700" };
                                    return (
                                        <div key={entry.id} className="flex gap-3 items-start">
                                            <div className="w-2 h-2 rounded-full bg-primary mt-2 shrink-0" />
                                            <div className="flex-1">
                                                <Badge variant="outline" className={`text-xs ${ec.color}`}>{ec.label}</Badge>
                                                {entry.notes && <p className="text-xs text-muted-foreground mt-1">{entry.notes}</p>}
                                                <p className="text-xs text-muted-foreground">{entry.updated_by_name} - {new Date(entry.timestamp).toLocaleString()}</p>
                                            </div>
                                        </div>
                                    );
                                })}
