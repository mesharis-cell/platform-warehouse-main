    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-4">
                ...
                <Select value={status || "_all_"} onValueChange={(v) => { setStatus(v === "_all_" ? "" : v); setPage(1); }}>
                    <SelectTrigger className="w-[180px]"><SelectValue placeholder="All statuses" /></SelectTrigger>
                    <SelectContent>
                        <SelectItem value="_all_">All statuses</SelectItem>
                        {Object.entries(PICKUP_STATUS_CONFIG).map(([k, c]) => (<SelectItem key={k} value={k}>{c.label}</SelectItem>))}
                    </SelectContent>
                </Select>
            </div>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {isLoading ? Array.from({ length: 3 }).map((_, i) => (
                                ...
                                <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No self-pickups found</TableCell></TableRow>
                            ) : pickups.map((p: any) => {
                                const sc = PICKUP_STATUS_CONFIG[p.self_pickup_status] || { label: p.self_pickup_status, color: "bg-gray-100 text-gray-700" };
                                const pw = p.pickup_window as any;
                                return (
                                    <TableRow key={p.id}>
                                        <TableCell><Link href={`/self-pickups/${p.id}`} className="font-medium text-primary hover:underline">{p.self_pickup_id}</Link></TableCell>
                                        <TableCell><span className="flex items-center gap-1"><User className="h-3 w-3 text-muted-foreground" />{p.collector_name}</span></TableCell>
                                        <TableCell>{pw?.start ? new Date(pw.start).toLocaleDateString() : "—"}</TableCell>
                                        <TableCell><Badge variant="outline" className={sc.color}>{sc.label}</Badge></TableCell>
                                        <TableCell>
                                            {p.self_pickup_status === "READY_FOR_PICKUP" && (
                                                <Link href={`/scanning/self-pickup-handover/${p.id}`}>
                                                    <Button size="sm" variant="outline">Handover Scan</Button>
                                                </Link>
                                            )}
                                            {p.self_pickup_status === "AWAITING_RETURN" && (
                                                <Link href={`/scanning/self-pickup-return/${p.id}`}>
                                                    <Button size="sm" variant="outline">Return Scan</Button>
                                                </Link>
                                            )}
                                        </TableCell>
                                    </TableRow>
